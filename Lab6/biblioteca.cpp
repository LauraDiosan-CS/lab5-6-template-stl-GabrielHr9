...
Carte::Carte(char* nume, char* autor, bool status)
{
	...
	strcpy_s(this->autor, strlen(nume) + 1, autor);
	this->status = status;
}
...
Carte::Carte(const Carte& c)
{
	...
	strcpy_s(this->autor, strlen(c.nume) + 1, c.autor);
...
{
	if (strcmp(this->nume, c.nume) == 0 && strcmp(this->autor, c.autor) == 0 && this->status == c.status) return true;
	else return false;
}
